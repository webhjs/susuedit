<template>
  <el-form label-width="80px" :model="formInline" :rules="rules" ref="attributeForm" :inline="true">
    ...
    <el-form-item label="日期类型" class="form-item">
      <el-select
        class="input-item"
    ...
    <el-form-item label="日期格式" class="form-item">
      <el-input class="input-item" v-model="formInline.dateformat" placeholder="请输入日期格式"></el-input>
    </el-form-item>
    <el-form-item label="最小日期" class="form-item">
      <el-date-picker
        class="input-item"
        v-model="formInline.min"
        :type="formInline.datetype"
        popper-class="bse-date-picker"
        :picker-options="minDisabledDate()"
        placeholder="选择日期时间"
      ></el-date-picker>
    </el-form-item>
    <el-form-item label="最大日期" class="form-item">
      <el-date-picker
        class="input-item"
        v-model="formInline.max"
        :type="formInline.datetype"
        popper-class="bse-date-picker"
        :picker-options="maxDisabledDate()"
        placeholder="选择日期时间"
      ></el-date-picker>
    </el-form-item>
  </el-form>
...
export default {
  ...
  methods: {
    switchDatetype(value) {
      switch (value) {
        case 'date':
          this.formInline.dateformat = 'yyyy-MM-dd'
          break
        case 'datetime':
          this.formInline.dateformat = 'yyyy-MM-dd HH:mm:ss'
          break
      }
    },
    minDisabledDate() {
      return {
        disabledDate: time => {
          return this.formInline.max && time.getTime() > new Date(this.formInline.max).getTime()
        }
      }
    },
    maxDisabledDate() {
      return {
        disabledDate: time => {
          return this.formInline.min && time.getTime() < new Date(this.formInline.min).getTime()
        }
      }
    }
  }
