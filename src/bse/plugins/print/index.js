...
class Print {
  constructor(dom, options) {
    ...
    if (typeof dom === 'string') {
      this.dom = this.conf.document.querySelector(dom)
    } else {
   ...
   */
  async run() {
    var content = await this.getStyle() + this.getHtml()
  ...
  getStyle() {
    var str = '',
      styles = this.conf.document.querySelectorAll('style,link')
  ...
  // 表单赋值
  getHtml() {
    var inputs = this.conf.document.querySelectorAll('input')
    var textareas = this.conf.document.querySelectorAll('textarea')
    var selects = this.conf.document.querySelectorAll('select')
    // debugger
  ...
  writeIframe(content) {
    ...
    var w,
      doc,
      iframe = this.conf.document.createElement('iframe'),
      f = this.conf.document.body.appendChild(iframe)
    console.log('f', f)
    ...
    iframe.onload = function () {
      ...
      _this.drawEchartImg(doc).then(() => {
        _this.toPrint(w)
        setTimeout(function () {
          this.conf?.document.body.removeChild(iframe)
          // 弹出后，回调
   ...
   */
  toPrint(frameWindow) {
    console.log('开始打印', frameWindow)
    try {
      setTimeout(function () {
        frameWindow.focus()
        try {
          if (!frameWindow.this.conf.document.execCommand('print', false, null)) {
            frameWindow.print()
          }
        } catch (e) {
          frameWindow.print()
  ...
  setDomHeight(arr) {
    if (arr && arr.length) {
      arr.forEach(name => {
        const domArr = this.conf.document.querySelectorAll(name)
        // debugger
        domArr.forEach(dom => {
